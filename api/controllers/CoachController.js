...
module.exports = {
  ...
  coachform: async function (req, res) {
    ...
    var dps = req.body.Coach.disciplines;
    if (dps) {
      if (typeof (dps) != "string") {
        ...
        dps.forEach(function (dp) {
          disciplines.push(dp);
          if (dp == '技巧體操'){
            req.body.Coach.Disciplines4 = 'ACRO';
          } else if (dp == '健美體操') {
            req.body.Coach.Disciplines6 = 'AER';
          } else if (dp == '男子競技體操') {
            req.body.Coach.Disciplines1 = 'MAG';
          } else if (dp == '女子競技體操') {
            req.body.Coach.Disciplines2 = 'WAG';
          } else if (dp == '藝術體操') {
            req.body.Coach.Disciplines3 = 'RG';
          } else if (dp == '彈網訓練') {
            req.body.Coach.Disciplines5 = 'TRA';
          }
        });
        req.body.Coach.disciplines = disciplines;
      } 
    }
  ...
  json: async function (req, res) {
    ...
    var everyones = await Coach.find().populate('user');
  ...
  confirm: async function (req, res) {
    ...
    if (req.method == 'POST') {
      ...
      await Coach.addToCollection(coach.id, "user").members(req.session.userId);
  ...
  //input pass or fail by coach
  passFail: async function (req, res) {

    if (req.method == 'GET') {
      var thatCourse = await Course.findOne(req.params.id);
      var results = await Results.find({ course: req.params.id }).populate('resultOwner');
      resultList = [];
      results.forEach(function (result) {
        switch (result.passFail) {
          case 0:
            result.passFail = '待處理';
            break;
          case 1:
      ...
      return res.view('coach/passFail', { course: thatCourse, results: resultList });
    }

    var records = req.body;

    for (var i = 0; i < records.length; i++) {
      var response = await Results.updateOne({ id: records[i].resultID }).set({ passFail: records[i].passFail });
      if (!response) {
        res.status(500);
        return res.json("Error updating Result: " + records[i]);
      }
      await Results.updateOne({ id: records[i].resultID }).set({ isVerified: 0 });
    }

    return res.ok();

    //return res.view('coach/passFail', { 'data': req.session.data });


  },

  //upload result and attendance sheet documents
  uploadDoc: async function (req, res) {

    var thatCourse = await Course.findOne(req.params.id);

    return res.view('coach/uploadDoc', { course: thatCourse });

    //return res.view('coach/uploadDoc', { 'data': req.session.data  });

  },

  //show the finish course list
  endList: async function (req, res) {

    // var course = await Course.findOne().populate("coach", req.session.userId);
    var coach = await User.findOne(req.session.userId).populate('taughtCourse');

    var courses = [];
    var verified = true;
    // var fstatus = [];

    for (var i = 0; i < coach.taughtCourse.length; i++) {
      var thatCourse = await Course.findOne(coach.taughtCourse[i].id).populate('participant');
      var results = await Results.find({ course: thatCourse.id }).populate('resultOwner');
      for (var y = 0; y < results.length; y++) {
        if (results[y].isVerified == 0 || results[y].isVerified == 2) {
          verified = false;
          if (results[y].isVerified == 2){
            thatCourse.fstatus = '已拒絕';
          } else if (results[y].passFail > 0 || (thatCourse.category == '普及體操' && thatCourse.docStatus > 0)) {
            thatCourse.fstatus = '已處理';
          } else {
            thatCourse.fstatus = '未處理';
          }
          break;
        } else {
          verified = true;
      ...
      if (thatCourse.status != 3) {
        // console.log(thatCourse);
        if (thatCourse.participant.length > 0 && !verified) {
          courses.push(thatCourse);
        }
      }
    };


    return res.view('coach/endCourseList', { course: courses });
